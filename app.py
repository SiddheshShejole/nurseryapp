        ...
        try:
            ...
            allproduct = Product.query.filter(
                or_(Product.category.like('%' + session['search'] + '%'),
                    Product.productname.like('%' + session['search'] + '%'),
                    Product.subcategory.like('%' + session['search'] + '%'))).all()
...
def add_to_cart():
    ...
    try:
        ...
        if session['search'] == '':
            ...
            allproduct = Product.query.filter(
                or_(Product.category.like('%' + session['search'] + '%'),
                    Product.subcategory.like('%' + session['search'] + '%'),
                    Product.productname.like('%' + session['search'] + '%'))).all()
...
def add_product():
    ...
    if request.method == "POST":
        try:
            ...
            result = Product(category=category, subcategory=subCategory, productname=productName,
                             rate=rate, image=image.filename)
            db.session.add(result)
            db.session.commit()
